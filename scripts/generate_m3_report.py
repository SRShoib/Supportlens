"""Evaluates all M3 transformer exports on the persisted test split, persists
EvalRun rows (CLAUDE.md rule #5: no metric without an eval run), benchmarks
CPU latency + export size for baseline and every transformer variant, and
renders docs/model-cards/*.md + docs/m3-comparison-report.md from the results
— nothing in either doc is hand-typed.

Run: uv run python scripts/generate_m3_report.py
"""
...
REPORT_PATH = ROOT / "docs" / "m3-comparison-report.md"

TASKS = ["intent", "urgency"]
DATASET_BY_TASK = {"intent": "bitext", "urgency": "twitter_slice_v1"}
BENCHMARK_TEXT_BY_TASK = {
    "intent": "I want to cancel my order and get a refund, please help me now.",
    "urgency": "This is unacceptable, I've been waiting 3 days and no one has responded!",
}
PREDICT_BATCH_SIZE = 64
WIN_MARGIN = 0.02  # macro-F1 delta above which we call it a real win, not noise
...
def _predict_in_batches(predictor: TransformerPredictor, texts: list[str]) -> list[str]:
    ...
    for start in range(0, len(texts), PREDICT_BATCH_SIZE):
        ...
        if start % (PREDICT_BATCH_SIZE * 20) == 0:
            print(f"    predicted {start + len(batch)}/{len(texts)}")
    return labels
...
def _render_model_card(result: VariantResult, baseline: BaselineResult) -> str:
    ...
    lines = [
        ...
        "## Data & splits",
        "",
        f"- Dataset: `{DATASET_BY_TASK[result.task]}`, split file `data/splits/{result.task}_v1.parquet`",
        "- Split: stratified 70/15/15 train/val/test, seed 42 (`ml/training/splits.py`) — the "
        "identical split M2's classical baseline trained on, so the comparison is apples-to-apples.",
    ...
    if result.task == "intent":
        lines.append(
            "- Fine-tuned on Bitext's synthetic, templated intent instructions — a near-ceiling "
            "test score reflects how well the model separates Bitext's 27 templates, not "
            "necessarily real customer-message understanding (see `docs/m2-baseline-report.md`'s "
            "qualitative spot-check for the same caveat on the classical baseline)."
        )
    else:
        lines.append(
            "- Trained on `weak_label_urgency()` rule-based labels, not human-verified ground "
            "truth — test macro-F1 measures agreement with the weak-label heuristic, not "
            "objective urgency. See the Cohen's kappa section of `docs/m2-baseline-report.md` for "
            "how the weak labels compare to a small LLM-labeled sample."
        )
    lines.append(
        f"- Truncates input to {hp['max_seq_length']} tokens; longer messages lose trailing context."
    )
    lines.append('- CPU-only inference (SPEC §5: "serving is CPU-only"), no quantization applied.')
    lines.append("")
    return "\n".join(lines)


def _render_comparison_report(
    baselines: dict[str, BaselineResult], variants: dict[str, list[VariantResult]]
) -> str:
    lines = [
        "# M3 comparison report: transformer fine-tunes vs classical baselines",
        "",
        "Generated by `scripts/generate_m3_report.py` from `eval_runs` rows persisted during this "
        "run plus the M2 baseline `eval_runs` rows — every number below comes from a committed "
        "eval run (CLAUDE.md rule #5), nothing here is hand-typed.",
        "",
    ]
    ...
    for task in TASKS:
        ...
        if delta > WIN_MARGIN:
            ...
            lines.append(
                ...
                f"(macro-F1 delta **{delta:+.4f}**). Reported honestly per SPEC M3's accept "
                "criterion rather than picking the fancier model regardless of the numbers."
            )
        lines.append("")
...
def main() -> None:
    ...
    REPORT_PATH.write_text(_render_comparison_report(baselines, variants), encoding="utf-8")
